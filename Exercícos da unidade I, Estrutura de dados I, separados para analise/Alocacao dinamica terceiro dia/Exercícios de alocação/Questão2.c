...
b) a porcentagem de aprovação, sabendo-se que a nota mínima para ser aprovado é*/
#include <stdio.h>
#include <stdlib.h>
#include <locale.h>
...
int main()
{
	...
	int N, quantidade; //Declaração das variáveis do número de questões e da quantidade de alunos.
   int i, j;
...
//alocação do gabarito.
	...
	for (i = 0; i < N; i++)//percorer cada questão do gabarito.
	{
...
// percorer o número de questões e lista-las
		for (j = 0; j < N; j++) // j é o responsável por percorrer o loop
		{
	...
	for (i = 0; i < quantidade; i++)
	{
		...
		{  // Loop para percorrer as questões.
			if (respostas_alunos[i * N + j] == gabarito[j])
			{
				...
				// com 10 dividido pelo número de questões na variável N.
			}
		}
